...
class Model {
 public:
  ...
  /**
   * @brief Метод CenterVertices отвечает за центрирование всех вершин модели по
   * отношению к центру координат системы.
   */
  void CenterVertices();
  ...
  /**
   * @brief Метод InitialScale отвечает за начальное масштабирование трехмерной
   * модели, чтобы модель корректно отображалась полностью.
   */
  void InitialScale();
  ...
  // void FindMaxValues(double* x_max, double* y_max, double* z_max);

  // void FindMinValues(double* x_min, double* y_min, double* z_min);
